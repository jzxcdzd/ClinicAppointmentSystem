...
export const initializeAppointmentsDb = () => {
  if (!localStorage.getItem("appointmentData")) {
    localStorage.setItem(
      "appointmentData",
      JSON.stringify(initialAppointmentData)
    );
  }
};
...
export const AppointmentDB = {
  ...
  getByPatientId: (patientId) =>
    getAppointmentData().filter((a) => a.patientId === patientId),
  getByPhysicianId: (physicianId) =>
    getAppointmentData().filter((a) => a.physicianId === physicianId),
  add: (newAppointment) => {
